...
class MainActivityViewModel(private val dustUseCase: DustUseCase) : ViewModel() {
    ...
    private val errorStatusLiveData = MutableLiveData<ApiStatus>()
    ...
    private fun loadTemp() {
        ...
        viewModelScope.launch() {
            ...
            try {
                errorStatusLiveData.value = ApiStatus.LOADING
                // 서버와 통신 성공한 경우
                if (dustUseCase.getDustInformation().isSuccessful) {
                    ...
                    errorStatusLiveData.value = ApiStatus.DONE
                }
            } catch (t: Throwable) {
                errorStatusLiveData.value = ApiStatus.ERROR
                Logger.v(t.message.toString())
    ...
    fun getErrorStatusLiveData() : LiveData<ApiStatus>{
